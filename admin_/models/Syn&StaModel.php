<?php
    ...
    function toltalQuantity(){
        $sql = "select sum(quantity) AS Total from bill_details";
        $totalPro =  pdo_query_value($sql);
        return $totalPro; 
    }function toltalPrice(){
        $sql = "select sum(price_product) AS TotalPrice from bill_details";
        $totalPro =  pdo_query_value($sql);
        return $totalPro; 
    }
    function orderNew(){
        $sql = "select count(id_bill) from bill where status = 0";
        $inStock=  pdo_query_value($sql);
        return $inStock; 
    }
    function orderPrepare(){
        $sql = "select count(id_bill) from bill where status = 1";
        $inStock =  pdo_query_value($sql);
        return $inStock; 
    }
    function orderDelivered(){
        $sql = "select count(id_bill) from bill where status = 2";
        $inStock =  pdo_query_value($sql);
        return $inStock; 
    }
    function orderSuccess(){
        $sql = "select count(id_bill) from bill where status = 3";
        $inStock =  pdo_query_value($sql);
        return $inStock; 
    }
    function OrderCanceled(){
        $sql = "select count(id_bill) from bill where status = 4";
        $inStock =  pdo_query_value($sql);
        return $inStock; 
    }